...
function AssignDeviceForm({ projectId, locations, state, onClose, onSave }) {
  const [locationId, setLocationId] = useState(locations[0]?.id || '');
  const [deviceIds, setDeviceIds] = useState([]);
  const candidates = state.devices.filter((device) => (
    device.archiveStatus !== '已作废'
    && device.id
    && device.sn
    && device.robotNo
    && device.deviceTypeId
    && (!device.projectId || device.projectId === projectId)
  ));
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="max-h-52 overflow-y-auto rounded-md border border-gray-200 p-2 space-y-1">
        ...
        {!candidates.length && <p className="py-4 text-center text-xs text-gray-400">暂无已完成建档且可绑定到当前项目的有效设备</p>}
      </div>
...
export default function ProjectDetail() {
  ...
  const [deviceFilters, setDeviceFilters] = useState({ query: '', locationId: '' });
  const project = state.projects.find((item) => item.id === id);

  const locations = state.locations.filter((item) => item.projectId === id);
  const activeLocations = locations.filter((item) => !item.disabled);
  const devices = state.devices.filter((item) => item.projectId === id);
  const filteredDevices = devices.filter((device) => {
    const query = deviceFilters.query.trim().toLowerCase();
    return (!query || `${device.sn || ''} ${device.robotNo || ''}`.toLowerCase().includes(query))
      && (!deviceFilters.locationId || device.locationId === deviceFilters.locationId);
  });
  const deliveries = state.deliveryPlans.filter((item) => item.projectId === id);
  ...
  const logs = state.operationLogs.filter((item) => item.projectId === id).sort((a, b) => (b.timestamp || '').localeCompare(a.timestamp || ''));
  ...
  if (!project) return <Page><PageHeader title={state.projectCenterLoading ? '项目加载中' : '项目不存在'} actions={<Btn as="link" to="/projects">返回项目列表</Btn>} /></Page>;
  const log = (actionType, notes) => dispatch({ type: 'ADD_OPERATION_LOG', payload: { id: createClientId('LOG', actionType), projectId: project.id, module: '项目中心', actionType, notes } });
  const saveProject = (form) => {
    const request = dispatch({ type: 'UPDATE_PROJECT', payload: { id: project.id, ...form, updatedAt: nowText() } });
    Promise.resolve(request).then((saved) => { if (saved) log('编辑项目', '更新项目基础信息'); });
    setModal(null);
  };
  const saveLocation = (form) => {
    const request = editLocation
      ? dispatch({ type: 'UPDATE_LOCATION', payload: { id: editLocation.id, ...form, updatedAt: nowText() } })
      : dispatch({ type: 'ADD_LOCATION', payload: { id: createClientId('LOC'), projectId: project.id, ...form, disabled: false, updatedAt: nowText() } });
    Promise.resolve(request).then((saved) => { if (saved) log(editLocation ? '编辑点位' : '新增点位', form.name); });
    setModal(null);
  };
  const disableLocation = (location) => {
    const request = dispatch({ type: 'UPDATE_LOCATION', payload: { id: location.id, disabled: true, updatedAt: nowText() } });
    Promise.resolve(request).then((saved) => { if (saved) log('停用点位', location.name); });
  };
  const assignDevices = (locationId, deviceIds) => {
    const requests = deviceIds.map((deviceId) => dispatch({ type: 'UPDATE_DEVICE', payload: { id: deviceId, projectId: project.id, locationId, preAssignedLocationId: locationId, updatedAt: nowText() } }));
    Promise.all(requests).then(() => log('绑定项目设备', `绑定 ${deviceIds.length} 台设备到点位 ${locations.find((item) => item.id === locationId)?.name}`));
    setModal(null);
  };
  ...
  return (
    <Page>
        ...
        <Section title={`项目设备（${devices.length}）`} right={<Btn size="sm" variant="primary" disabled={!activeLocations.length} onClick={() => setModal({ type: 'assign' })}>绑定设备</Btn>} bodyClassName="p-0">
          <div className="flex flex-wrap items-center gap-2 border-b border-gray-200 px-3 py-2.5">
            <Input
              ...
              onChange={(event) => setDeviceFilters((prev) => ({ ...prev, query: event.target.value }))}
            />
            <Select
              className="w-56"
              value={deviceFilters.locationId}
              onChange={(event) => setDeviceFilters((prev) => ({ ...prev, locationId: event.target.value }))}
            >
              <option value="">全部点位</option>
              {locations.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}
            </Select>
            <span className="ml-auto text-xs text-gray-400">当前展示 {filteredDevices.length} / {devices.length} 台</span>
          </div>
        ...
        <Section title="交付异常" bodyClassName="p-0">
          <Table head={['来源批次', '来源记录标题', '来源类型', '关联设备', '异常说明', '记录人', '记录时间', '操作']} empty="暂无交付异常">
            {exceptions.map((item) => {
              ...
              const batchUrl = item.batchId ? `/delivery-plans/${item.deliveryPlanId}/batches/${item.batchId}?returnTo=${encodeURIComponent(`/projects/${project.id}?tab=delivery`)}#exceptions` : `/delivery-plans/${item.deliveryPlanId}?tab=exceptions&returnTo=${encodeURIComponent(`/projects/${project.id}?tab=delivery`)}`;
              return <tr key={item.id} className="hover:bg-[#fafafa]">
              <td className="px-3 py-2">{item.batchId ? <Link className="ui-link text-[13px]" to={batchUrl}>{batch ? batchDisplayName(batch) : item.batchId}</Link> : '—'}</td>
      ...
      <Modal isOpen={modal?.type === 'assign'} onClose={() => setModal(null)} title="绑定项目设备"><AssignDeviceForm projectId={project.id} locations={activeLocations} state={state} onClose={() => setModal(null)} onSave={assignDevices} /></Modal>
    </Page>
  );
}
