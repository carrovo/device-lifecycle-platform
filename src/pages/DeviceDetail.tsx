...
function FeishuTables({ device }) {
  const tables = (device.otherFeishuTables || []).filter((item) => item.name && !/^\d+$/.test(item.name.trim()) && item.url);
  return (
    <Section title="相关飞书表格">
      {tables.length ? <div className="divide-y divide-gray-100">
        {tables.map((item) => <div key={item.id} className="flex items-start justify-between gap-3 py-2"><div><span className="text-[13px] text-gray-700">{item.name}</span>{item.note && <p className="text-xs text-gray-400 mt-0.5">{item.note}</p>}</div><a className="ui-link text-[13px]" href={item.url} target="_blank" rel="noreferrer">打开表格</a></div>)}
      </div> : <p className="py-2 text-xs text-gray-400">暂无飞书表格。</p>}
    </Section>
  );
}
    ...
    <Page>
      ...
      {active === 'overview' && <>
        <Section title="建档信息"><DescList cols={3} items={[
          ['建档人', device.assembler || '—'], ['建档时间', device.createdAt || '—'], ['最近更新时间', device.updatedAt || '—'], ['档案状态', device.archiveStatus || '有效'],
        ]} /></Section>
        <Section title="ERP 产品入库关联">
          {(device.erpInboundLinks || []).length ? <div className="space-y-3">
            <DescList cols={3} items={[
              ['关联情况', `已关联 ${(device.erpInboundLinks || []).length} 条入库明细`],
              ['平台设备 SN', device.sn], ['机器人编号', device.robotNo],
              ['最近核验时间', device.erpInboundVerifiedAt || device.erpInboundMatchedAt || '—'],
              ['来源信息', <Link className="ui-link" to="/erp-center?tab=list&type=productInbound">查看 ERP 单据</Link>],
            ]} />
            <Table head={['产品入库单', 'ERP 明细 ID', '批次号', '产品', 'ERP 序列号', '仓库', '入库时间']} empty="暂无关联明细">
              {(device.erpInboundLinks || []).map((link) => <tr key={link.recordId} className="hover:bg-[#fafafa]">
                <td className="px-3 py-2 font-mono text-xs">{link.inboundNo || '—'}</td><td className="px-3 py-2 font-mono text-xs">{link.recordId || '—'}</td>
                <td className="px-3 py-2 font-mono text-xs">{link.batchNo || '—'}</td><td className="px-3 py-2">{link.productName || link.productCode || '—'}</td>
                <td className="px-3 py-2 font-mono text-xs">{link.serialNo || '—'}</td><td className="px-3 py-2">{link.warehouse || '—'}</td><td className="px-3 py-2 text-xs text-gray-500">{link.inboundTime || '—'}</td>
              </tr>)}
            </Table>
          </div> : <p className="text-[13px] text-gray-500">当前设备尚未匹配到 ERP 产品入库记录。</p>}
        </Section>
      </>}
