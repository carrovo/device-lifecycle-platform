...
import { afterSalesCloseReasons, type AfterSalesOrder } from '../../data/afterSalesOrders';
...
export default function MobileAfterSalesOrderDetail() {
  ...
  return <MobileFrame>
    ...
    <div className="space-y-3 p-4 pb-28">
      ...
      {order.status === '现场处理中' && <>
        <CompleteOrderForm key={`close-${order.updatedAt}`} order={order} issue={issue} currentUser={state.currentUser || '当前用户'} dispatch={dispatch} onClose={(patch) => updateOrder(patch, '完成并关单', '工单状态：现场处理中 → 已关单；售后现场处理已完成。')} />
      </>}
...
type CompleteOrderFormProps = {
  ...
  onClose: (patch: Partial<AfterSalesOrder>) => void;
};
function CompleteOrderForm({ order, issue, currentUser, dispatch, onClose }: CompleteOrderFormProps) {
  ...
  const close = () => {
    if (!values.causeLevel1.trim()) return setMessage('请选择最终故障分类。');
    if (!values.actualSolution.trim()) return setMessage('请填写实际处理方案。');
    ...
    const missing = afterSalesCloseReasons({ ...order, ...handling, materials });
    if (missing.length) return setMessage(`暂不能关单：${missing.join('、')}。`);
    onClose({ ...handling, status: '已关单', closedBy: currentUser, closedAt: nowText(), materials });
  ...
  return <Section title="完成现场处理并关单"><div className="space-y-4"><p className="text-xs leading-5 text-gray-500">关单只结束本次现场服务，不会修改来源问题的质量跟进字段。</p><Field label="实际处理方案" required><Textarea value={values.actualSolution} onChange={(event) => set('actualSolution', event.target.value)} /></Field>
    ...
    <div className="border-t border-gray-100 pt-4"><AttachmentUpload value={proof} onChange={setProof} label="正常工作视频或处理结果证明资料" required /><div className="mt-3"><AttachmentList items={order.materials} empty="暂无现场资料。" /></div></div>
    {message && <p className="rounded-md bg-amber-50 px-3 py-2 text-xs text-amber-800">{message}</p>}<button type="button" onClick={close} className="min-h-12 w-full rounded-md bg-gray-900 text-sm font-medium text-white">完成处理并关单</button>
  </div></Section>;
}
...
function ReadOnlyResult({ order, issue }: { order: AfterSalesOrder; issue?: IssueRecord }) {
  ...
  return <><Section title="现场处理结果"><DetailRows items={[
    ['接单人 / 时间', `${dash(order.engineer)} / ${dash(order.acceptedAt)}`], ['实际上门时间', dash(order.actualVisitAt)], ['实际处理方案', dash(order.actualSolution)], ...(order.finalResult ? [['历史最终处理结果', order.finalResult] as [string, ReactNode]] : []), ['最终故障分类', [issue?.causeLevel1, issue?.causeLevel2, issue?.causeLevel3].filter(Boolean).join(' / ') || '—'], ['是否涉及换件', order.involvesReplacement ? '是' : '否'], ...replacementItems,
  ...
  {order.status === '已关单' && <Section title="关单信息"><DetailRows items={[["关单人", dash(order.closedBy)], ['关单时间', dash(order.closedAt)], ...(order.closeNote ? [['历史关单说明', order.closeNote] as [string, ReactNode]] : [])]} /></Section>}
