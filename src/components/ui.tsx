...
import { Children, useState, type InputHTMLAttributes, type ReactNode, type SelectHTMLAttributes } from 'react';
...
export function Section({ title, subtitle, right, children, className = '', bodyClassName = 'p-4', ...rest }: { title?: ReactNode; subtitle?: ReactNode; right?: ReactNode; children: ReactNode; className?: string; bodyClassName?: string; [key: string]: unknown }) {
  return (
...
export function Btn({ variant = 'secondary', size = 'md', as = 'button', to, className = '', children, ...rest }: { variant?: keyof typeof BTN_VARIANT; size?: keyof typeof BTN_SIZE; as?: 'button' | 'link'; to?: string; className?: string; children: ReactNode; [key: string]: unknown }) {
...
// 行内文字操作（表格操作列 / 详情跳转）
export function LinkAction({ to, onClick, children, className = '' }: { to?: string; onClick?: () => void; children: ReactNode; className?: string }) {
...
/* ── 中性 chip / tag（非状态语义） ─────────────────── */
export function Chip({ children, tone = 'neutral', className = '' }: { children: ReactNode; tone?: 'neutral' | 'solid' | 'outline'; className?: string }) {
  const tones = {
...
/* ── 指标卡（紧凑，突出数字与状态） ─────────────────── */
export function StatCard({ label, value, hint, tone = 'default' }: { label: ReactNode; value: ReactNode; hint?: ReactNode; tone?: 'default' | 'danger' | 'warning' | 'success' }) {
  const valTone = {
...
export function CompactProgress({ value = 0, total = 0, className = '' }: { value?: number; total?: number; className?: string }) {
...
/* ── 详情键值区（不把字段堆成一整屏表单） ───────────── */
export function DescList({ items, cols = 2, className = '' }: { items: Array<readonly [ReactNode, ReactNode] | null | undefined | false>; cols?: 1 | 2 | 3 | 4; className?: string }) {
  ...
  return (
    <dl className={`grid grid-cols-1 ${colClass} gap-x-8 gap-y-3 ${className}`}>
      {items.filter((item): item is readonly [ReactNode, ReactNode] => Boolean(item)).map(([label, value], i) => (
...
/* ── 数据表格（紧凑、细分割线、浅表头） ───────────────── */
export function Table({ head, children, empty = '暂无数据', footer, className = '', tableClassName = '' }: { head: ReactNode[]; children: ReactNode; empty?: ReactNode; footer?: ReactNode; className?: string; tableClassName?: string }) {
...
export function Stepper({ steps, current = 0, className = '' }: { steps: Array<{ key?: string; label: string }>; current?: string | number; className?: string }) {
