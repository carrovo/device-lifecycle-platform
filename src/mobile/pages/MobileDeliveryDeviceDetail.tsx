...
function InstallationModal({ isOpen, record, onClose, onSave }: { isOpen: boolean; record: DeliverySubOrder['devices'][number]; onClose: () => void; onSave: (status: string, note: string, attachment: string) => void }) {
  ...
  return <Modal isOpen={isOpen} onClose={onClose} title="更新安装调试"><div className="space-y-4"><Field label="安装调试状态" required><select value={status} onChange={(event) => setStatus(event.target.value)} className="min-h-11 w-full rounded-md border border-gray-300 bg-white px-3 text-sm text-gray-800">{INSTALLATION_STATUSES.map((item) => <option key={item}>{item}</option>)}</select></Field><Field label="执行说明"><Textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder="记录安装、调试和现场处理情况" /></Field><AttachmentUpload value={attachment} onChange={setAttachment} label="现场资料" /><div className="flex gap-2"><button type="button" onClick={onClose} className="min-h-11 flex-1 rounded-md border border-gray-300 text-sm text-gray-600">取消</button><button type="button" onClick={() => onSave(status, note, attachment)} className="min-h-11 flex-1 rounded-md bg-gray-900 text-sm font-medium text-white">保存安装调试</button></div></div></Modal>;
}
