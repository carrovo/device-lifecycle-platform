...
const TONE_GROUPS = {
  success: [
    '已完成', '已完成测试', '已完成复测', '生产已完成', '安装调试完成', 'Pass', '通过', '已通过', '测试通过', '复测通过', '检验合格', '合格',
    '已入库', '已关单', '已验收', '已交付', '在线运营', '已解决', '已处理', '有效', '已齐套', '已装配',
    '可交付', '已出厂', '无明显卡点', '已换件', '已返修', '已更换设备', '在线', '有效', '已建档', '已维护验收', '已进入生产',
    // 平台占用状态
    '在库可用',
    // 问题池（技术客服预处理）
    '已远程关闭', '远程已解决', '是',
  ...
  info: [
    '进行中', '执行中', '处理中', '生产中', '交付中', '现场处理中', '装配中', '整机装配', '模块绑定中', '模块绑定',
    ...
    '已绑定设备',
    // 问题池 / 交付异常（预处理进行中）
    '已转售后工单', '预处理中', '技术客服预处理中', '交付侧处理中', '根因分析进行中',
    // 问题来源 / 类型
    '扫码上报', '使用问题',
    // 故障原因
    '硬件',
  ...
  warning: [
    '预警', '长期未结', '当前卡点', '延期', '已延期', '待验收', '待设备验收', '待复测', '待检', '特批使用', '有条件通过', '待客户确认',
  ...
  danger: [
    ...
    '生产返修中', '生产返修', '已记录异常', '阻塞', '超时', '节点超时', '同步异常', '退货换货', '装配异常', '安装异常',
  ...
  neutral: [
    '待处理', '暂无结果', '待分派', '待接单', '待上门', '未接入', '未知', '离线', '未开始', '未测试', '待测试', '待问题池接入', '不适用', '未执行（选填）',
    ...
    '未关联',
    // 问题池 / 交付异常（待办/退回/已记录）
    '待预处理', '已记录', '已退回交付继续处理', '退回交付继续处理', '待后续处理', '有异常记录',
    // 平台占用状态（历史/中性）
    '已更换',
    // 问题来源 / 类型 / 故障原因（中性）
    '手动录入', '使用', '重复',
