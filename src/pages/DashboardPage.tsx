...
function AfterSalesDashboard({ state }: { state: any }) {
  ...
  return <div className="space-y-4">
    ...
    <Section title="近期关单记录" bodyClassName="p-0"><Table head={['售后工单', '设备', '来源问题', '实际处理方案', '关单时间', '操作']} empty="暂无关单记录。">{closedOrders.slice().sort((a, b) => (b.closedAt || '').localeCompare(a.closedAt || '')).map((order) => <tr key={order.id}><td className="whitespace-nowrap px-3 py-2 font-mono text-xs">{order.orderNo}</td><td className="px-3 py-2 font-mono text-xs">{order.snapshot.deviceIdentifier || '—'}</td><td className="whitespace-nowrap px-3 py-2 font-mono text-xs">{order.issueNo}</td><td className="px-3 py-2">{order.actualSolution || order.finalResult || '—'}</td><td className="whitespace-nowrap px-3 py-2 text-xs">{order.closedAt || '—'}</td><td className="whitespace-nowrap px-3 py-2"><Link className="ui-link text-[13px]" to={`/after-sales/orders/${order.id}`}>查看工单</Link></td></tr>)}</Table></Section>
  </div>;
}
