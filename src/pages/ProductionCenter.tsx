...
import { getProductionDevicePage, getProductionDeviceTypes } from '../api/production';
...
const modelName = (device, types) => (Array.isArray(types) ? types : []).find((item) => item.id === device.deviceTypeId)?.name || '—';
...
function StandardFlow({ current = '' }: { current?: string | number }) {
  return (
...
function DeviceForm({ device = null, correction = false, deviceTypes, devices, onClose, onSave }) {
  ...
  const submit = () => {
    ...
    if (correction && !form.correctionReason.trim()) {
      setError('更正设备信息时必须填写更正原因。');
      return;
    }
    const duplicate = devices.some((item) => item.id !== device?.id
      && (String(item.sn || '').toLowerCase() === form.sn.trim().toLowerCase()
        || String(item.robotNo || '').toLowerCase() === form.robotNo.trim().toLowerCase()));
    if (duplicate) {
  ...
  return (
    <div className="space-y-5">
      <p className="text-xs text-gray-500">{correction
        ? '可更正设备 SN、机器人编号或设备型号；SN 和机器人编号在全平台内不能重复。'
        : '设备进入立腿状态后建档。设备 SN 是履历主识别口径，机器人编号是唯一辅助标识。'}</p>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className="block text-xs text-gray-600 mb-1">设备 SN <span className="text-red-500">*</span></label>
          <Input className="w-full" value={form.sn} onChange={(event) => update('sn', event.target.value)} placeholder="用于识别设备完整履历" />
        </div>
        <div>
          <label className="block text-xs text-gray-600 mb-1">机器人编号 <span className="text-red-500">*</span></label>
          <Input className="w-full" value={form.robotNo} onChange={(event) => update('robotNo', event.target.value)} placeholder="同一设备的唯一辅助标识" />
        </div>
      </div>
      <div>
...
function ArchiveTab({ state, dispatch, setSearchParams }) {
  ...
  const updateFilter = (key, value) => { setFilters((prev) => ({ ...prev, [key]: value })); setResult((prev) => ({ ...prev, page: 1 })); };
  useEffect(() => {
    let cancelled = false;
    getProductionDeviceTypes()
      .then((payload) => { if (!cancelled) setTypes(Array.isArray(payload) ? payload : []); })
      .catch(() => {});
    return () => { cancelled = true; };
  }, []);
  useEffect(() => {
    let cancelled = false;
    const timer = window.setTimeout(() => getProductionDevicePage({ ...filters, scope: 'archive', page: result.page, size: result.size })
      .then((payload) => {
        if (!cancelled) setResult((prev) => ({ ...prev, ...payload, items: Array.isArray(payload?.items) ? payload.items : [] }));
      }).catch(() => { if (!cancelled) setResult((prev) => ({ ...prev, items: [] })); }), filters.query ? 250 : 0);
    return () => { cancelled = true; window.clearTimeout(timer); };
  }, [filters, result.page, result.size, reload]);
  const rows = Array.isArray(result.items) ? result.items : [];
  const selected = modal?.id ? rows.find((item) => item.id === modal.id) : null;

  const addLog = (deviceId, actionType, notes, timestamp = nowText()) => {
    dispatch({ type: 'ADD_OPERATION_LOG', payload: {
      id: createClientId('LOG', deviceId), deviceId, operator: state.currentUser, timestamp,
      actionType, module: '生产中心', notes,
    } });
  };
  const createDevice = (form) => {
    setSaveError('');
    const firstNode = state.productionSteps?.[0];
    if (!firstNode) {
      setSaveError('生产节点字典尚未加载，暂不能创建设备。');
      return;
    }
    const createdAt = nowText();
    const id = createClientId('DEV');
    dispatch({ type: 'ADD_DEVICE', payload: {
      id, sn: form.sn, robotNo: form.robotNo, deviceTypeId: form.deviceTypeId,
      status: firstNode.label, productionStatus: firstNode.key, currentProductionNode: firstNode.key,
      productionComplete: false, repairStatus: null, archiveStatus: '有效',
      assembler: state.currentUser, createdAt, updatedAt: createdAt, projectId: null, locationId: null,
      electronicAcceptanceUrl: '', otherFeishuTables: [], deliveryPlanIds: [],
      productionHistory: [{
        id: `PH-${id}-archive`, node: firstNode.key, nodeLabel: firstNode.label, recordType: 'node',
        result: '已建档', operator: state.currentUser, time: createdAt,
        summary: `设备进入${firstNode.label}，建立平台设备档案`,
      }],
    } }).then((created) => {
      addLog(created.id, '设备建档', `建立设备档案 ${created.sn}`, createdAt);
      setModal({ type: 'created', id: created.id, sn: created.sn });
      setReload((value) => value + 1);
    }).catch((error) => {
      // Keep the form open and show the backend validation message next to the action.
  ...
  const correctDevice = (form) => {
    ...
    const timestamp = nowText();
    const changes = [];
    if (selected.sn !== form.sn) changes.push(`设备 SN：${selected.sn} → ${form.sn}`);
    if (selected.robotNo !== form.robotNo) changes.push(`机器人编号：${selected.robotNo} → ${form.robotNo}`);
    if (selected.deviceTypeId !== form.deviceTypeId) {
      changes.push(`设备型号：${modelName(selected, types)} → ${types.find((item) => item.id === form.deviceTypeId)?.name || form.deviceTypeId}`);
    }
    dispatch({ type: 'UPDATE_DEVICE', payload: {
      id: selected.id,
      sn: form.sn,
      ...
      addLog(selected.id, '更正设备信息', `${changes.join('；') || '设备信息未变化'}；原因：${form.correctionReason}`, timestamp);
      setModal(null);
      setReload((value) => value + 1);
    }).catch((error) => setSaveError(error.message));
  };
  const removeDevice = () => {
    dispatch({ type: 'DELETE_DEVICE', payload: selected.id }).then(() => {
      setModal(null);
      setReload((value) => value + 1);
    }).catch(() => {});
  };
    ...
    <>
      <Section title="设备建档" subtitle="设备进入立腿状态后，由人员在平台建立档案">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <Btn variant="primary" onClick={() => { setSaveError(''); setModal({ type: 'create' }); }}>新建设备</Btn>
        </div>
      </Section>
      <Toolbar right={<span className="text-xs text-gray-400">共 {result.total} 台设备</span>}>
        ...
        <Select value={filters.model} onChange={(event) => updateFilter('model', event.target.value)}>
          <option value="">全部设备型号</option>{types.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}
        </Select>
      ...
      <Table
        ...
        empty="暂无设备档案"
        footer={<Pagination page={result.page} pageSize={result.size} total={result.total} totalPages={result.totalPages} onChange={(page) => setResult((prev) => ({ ...prev, page }))} onPageSizeChange={(size) => setResult((prev) => ({ ...prev, page: 1, size }))} />}
      >
        {rows.map((device) => (
          <tr key={device.id} className="hover:bg-[#fafafa]">
            ...
            <td className="px-3 py-2 text-gray-700 whitespace-nowrap">{modelName(device, types)}</td>
            ...
            <td className="px-3 py-2 whitespace-nowrap">
              <div className="flex items-center gap-3">
                <LinkAction onClick={() => { setSaveError(''); setModal({
      ...
      <Modal isOpen={modal?.type === 'create'} onClose={() => setModal(null)} title="新建设备档案" size="lg">
        {saveError && <p className="mb-3 rounded-md border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-700">{saveError}</p>}
        <DeviceForm devices={rows} deviceTypes={types} onClose={() => setModal(null)} onSave={createDevice} />
      </Modal>
      <Modal isOpen={modal?.type === 'edit'} onClose={() => setModal(null)} title="编辑设备档案" size="lg">
        {saveError && <p className="mb-3 rounded-md border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-700">{saveError}</p>}
        {selected && <DeviceForm device={selected} devices={rows} deviceTypes={types} onClose={() => setModal(null)} onSave={updateDevice} />}
      </Modal>
      <Modal isOpen={modal?.type === 'correct'} onClose={() => setModal(null)} title="更正设备信息" size="lg">
        {saveError && <p className="mb-3 rounded-md border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-700">{saveError}</p>}
        {selected && <DeviceForm correction device={selected} devices={rows} deviceTypes={types} onClose={() => setModal(null)} onSave={correctDevice} />}
      </Modal>
      <Modal isOpen={modal?.type === 'view'} onClose={() => setModal(null)} title="设备建档信息">
        {selected && <DescList cols={2} items={[
          ...
          ['设备型号', modelName(selected, types)], ['建档人', selected.assembler],
...
function NonTestForm({ device, onSave, submitting = false }) {
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="rounded-md border border-gray-200 p-4 space-y-3">
          ...
          <Btn disabled={submitting} onClick={recordException}>{submitting ? '正在提交…' : '记录当前节点异常'}</Btn>
        </div>
        <div className="rounded-md border border-gray-200 p-4 space-y-3">
          ...
          <Btn variant="primary" disabled={submitting} onClick={complete}>{submitting ? '正在提交…' : '确认完成当前节点'}</Btn>
        </div>
      </div>
      {error && <p className="text-xs text-red-600">{error}</p>}
    </div>
...
function TestForm({ node, onSave, submitting = false }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {error && <p className="text-xs text-red-600">{error}</p>}
      <div className="flex justify-end"><Btn variant="primary" disabled={submitting} onClick={submit}>{submitting ? '正在提交…' : `提交${productionLabel(node)}结果`}</Btn></div>
    </div>
  );
}

function RepairForm({ onSave, submitting = false }) {
  const [form, setForm] = useState({
  ...
  return (
    <div className="space-y-4">
      ...
      {error && <p className="text-xs text-red-600">{error}</p>}
      <div className="flex justify-end"><Btn variant="primary" disabled={submitting} onClick={submit}>{submitting ? '正在提交…' : '提交返修及复测结果'}</Btn></div>
    </div>
  );
}

function RevisionForm({ record, onClose, onSave, submitting = false }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-end gap-2"><Btn disabled={submitting} onClick={onClose}>取消</Btn><Btn variant="primary" disabled={!reason.trim() || submitting} onClick={() => onSave({ result, summary, reason })}>{submitting ? '正在提交…' : '提交结果修订'}</Btn></div>
    </div>
  );
}

function CorrectionNoteForm({ onClose, onSave, submitting = false }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-end gap-2"><Btn disabled={submitting} onClick={onClose}>取消</Btn><Btn variant="primary" disabled={!note.trim() || submitting} onClick={() => onSave(note.trim())}>{submitting ? '正在提交…' : '追加更正说明'}</Btn></div>
    </div>
  );
}
...
function FeishuRelations({ device, setModal }) {
  const tables = device.otherFeishuTables || [];
  return (
    <div>
      <div className="flex items-center justify-between gap-3 mb-3">
        <h3 className="text-[13px] font-semibold text-gray-800">相关飞书表格</h3>
        <Btn size="sm" onClick={() => setModal({ type: 'otherFeishu' })}>添加飞书表格</Btn>
      </div>
      {tables.length ? (
        ...
        <p className="py-3 text-xs text-gray-400">暂无飞书表格，点击右上角“添加飞书表格”进行添加。</p>
      )}
    </div>
  );
}
...
function ErpRelation({ device }) {
  const links = device.erpInboundLinks || [];
  return (
      ...
      {links.length ? (
        <div className="space-y-3">
          <DescList cols={3} items={[
            ['关联情况', `已关联 ${links.length} 条入库明细`], ['平台设备 SN', device.sn],
            ['机器人编号', device.robotNo], ['最近核验时间', device.erpInboundVerifiedAt || device.erpInboundMatchedAt || '—'],
          ]} />
          <Table head={['产品入库单', '批次号', '产品', 'ERP 序列号', '仓库', '入库时间']} empty="暂无关联明细">
            {links.map((link) => <tr key={link.recordId} className="hover:bg-[#fafafa]">
              <td className="px-3 py-2 font-mono text-xs">{link.inboundNo || '—'}</td><td className="px-3 py-2 font-mono text-xs">{link.batchNo || '—'}</td>
              <td className="px-3 py-2">{link.productName || link.productCode || '—'}</td><td className="px-3 py-2 font-mono text-xs">{link.serialNo || '—'}</td>
              <td className="px-3 py-2">{link.warehouse || '—'}</td><td className="px-3 py-2 text-xs text-gray-500">{link.inboundTime || '—'}</td>
            </tr>)}
          </Table>
        </div>
      ) : isProductionComplete(device) ? (
...
function FlowDetail({ device, state, dispatch, setSearchParams }) {
  ...
  const addLog = (actionType, notes, fromStatus = productionProgressLabel(device), toStatus = fromStatus, timestamp = nowText()) => {
    dispatch({ type: 'ADD_OPERATION_LOG', payload: {
      id: createClientId('LOG'), deviceId: device.id, operator: state.currentUser, timestamp,
      actionType, module: '生产中心', fromStatus, toStatus, notes,
    } });
  };
  const submitting = submission.status === 'loading';
  useEffect(() => {
    if (!['success', 'error'].includes(submission.status)) return undefined;
    const timer = window.setTimeout(
      () => setSubmission({ status: 'idle', message: '' }),
      submission.status === 'success' ? 3500 : 6000,
    );
    return () => window.clearTimeout(timer);
  }, [submission.status, submission.message]);

  const updateDevice = (payload) => dispatch({
    type: 'UPDATE_DEVICE_CONFIRMED',
    payload: { id: device.id, ...payload },
  });
  const persistDevice = async (payload, successMessage) => {
    if (submitting) return false;
    setSubmission({ status: 'loading', message: '正在提交生产流转数据…' });
    try {
      await updateDevice(payload);
      setSubmission({ status: 'success', message: successMessage });
      return true;
    } catch (error) {
      setSubmission({ status: 'error', message: `提交失败：${error.message}` });
      return false;
    }
  };

  const saveNonTest = async (form) => {
    const timestamp = nowText();
    if (form.action === 'exception') {
      const record = {
        id: createClientId('PH', device.id), node: currentKey, nodeLabel: productionLabel(currentKey),
        recordType: 'node', result: '已记录异常', summary: form.exceptionNote,
        operator: state.currentUser, time: timestamp,
      };
      const saved = await persistDevice(
        { productionHistory: [...history, record], updatedAt: timestamp },
        `${productionLabel(currentKey)}异常已提交成功`,
      );
      if (!saved) return;
    ...
    const saved = await persistDevice({
      ...
      productionHistory: [...history, record], updatedAt: timestamp,
    }, `${productionLabel(currentKey)}已提交成功${next ? `，已进入${productionLabel(next)}` : ''}`);
    if (!saved) return;
  ...
  const saveTest = async (form) => {
    ...
    const saved = await persistDevice({
      ...
      repairStatus: form.result === 'NG' && form.repairRequired ? 'inProgress' : null,
      productionHistory: [...history, record], updatedAt: timestamp,
    }, `${productionLabel(currentKey)}结果提交成功：${form.result}`);
    if (!saved) return;
  ...
  const saveRepair = async (form) => {
    ...
    const record = {
      id: createClientId('PH', device.id), node: failedNode, nodeLabel: `${productionLabel(failedNode)}返修 / 复测`,
  ...
  const reviseResult = async ({ result, summary, reason }) => {
    ...
    const saved = await persistDevice({
      ...
      repairStatus: result === 'NG' ? 'inProgress' : getRepairStatus(device) === 'inProgress' ? null : getRepairStatus(device),
      updatedAt: timestamp,
    }, `${productionLabel(record.node)}测试结果修订成功`);
    if (!saved) return;
  ...
  const addCorrection = async (note) => {
    const timestamp = nowText();
    const record = modal.record;
    const revisedHistory = history.map((item) => item.id === record.id
      ...
      : item);
    const saved = await persistDevice(
      { productionHistory: revisedHistory, updatedAt: timestamp },
      `${productionLabel(record.node)}更正说明追加成功`,
    );
    if (!saved) return;
  ...
  const saveOtherFeishu = (form) => {
    ...
    const records = existing
      ...
      : [...(device.otherFeishuTables || []), { id: createClientId('FS', device.id), ...form }];
    ...
    <>
      {submission.status !== 'idle' && (
        <div
          role="status"
          aria-live="polite"
          className={`fixed right-6 top-6 z-[100] max-w-sm rounded-lg border px-4 py-3 text-[13px] shadow-lg ${
      ...
      {!completed && (
        <Section title={`当前生产操作 · ${repairStatus === 'inProgress' ? '返修及复测' : productionLabel(currentKey)}`}>
          {repairStatus === 'inProgress'
            ? <RepairForm submitting={submitting} onSave={saveRepair} />
            : isTestNode(currentKey)
              ? <TestForm node={currentKey} submitting={submitting} onSave={saveTest} />
              : <NonTestForm device={device} submitting={submitting} onSave={saveNonTest} />}
        </Section>
      )}
      ...
      <Modal isOpen={modal?.type === 'revision'} onClose={() => setModal(null)} title="修订测试结果" size="lg">
        {modal?.record && <RevisionForm record={modal.record} submitting={submitting} onClose={() => setModal(null)} onSave={reviseResult} />}
      </Modal>
      <Modal isOpen={modal?.type === 'correction'} onClose={() => setModal(null)} title="追加节点更正说明">
        <CorrectionNoteForm submitting={submitting} onClose={() => setModal(null)} onSave={addCorrection} />
      </Modal>
      <Modal isOpen={modal?.type === 'otherFeishu'} onClose={() => setModal(null)} title={modal?.record ? '编辑飞书表格' : '添加飞书表格'} size="lg">
        <OtherFeishuForm record={modal?.record} onClose={() => setModal(null)} onSave={saveOtherFeishu} />
      </Modal>
      <Modal isOpen={modal?.type === 'deleteOtherFeishu'} onClose={() => setModal(null)} title="删除飞书表格">
        <div className="space-y-4">
...
function FlowList({ setSearchParams, ..._rest }) {
  const [workspace, setWorkspace] = useState('inProcess');
  const [filters, setFilters] = useState({ query: '', model: '', node: '', exception: '', repair: '', sort: 'desc' });
  const [types, setTypes] = useState([]);
  const [result, setResult] = useState({ items: [], page: 1, size: 10, total: 0, totalPages: 1 });
  const update = (key, value) => { setFilters((prev) => ({ ...prev, [key]: value })); setResult((prev) => ({ ...prev, page: 1 })); };
  useEffect(() => {
    let cancelled = false;
    getProductionDeviceTypes().then((payload) => {
      if (!cancelled) setTypes(Array.isArray(payload) ? payload : []);
    }).catch(() => {});
    return () => { cancelled = true; };
  }, []);
  useEffect(() => {
    let cancelled = false;
    const timer = window.setTimeout(() => getProductionDevicePage({
      query: filters.query, model: filters.model, status: filters.node, repair: filters.repair,
      exception: filters.exception, scope: workspace, sort: filters.sort, page: result.page, size: result.size,
    }).then((payload) => {
      if (!cancelled) setResult((prev) => ({ ...prev, ...payload, items: Array.isArray(payload?.items) ? payload.items : [] }));
    }).catch(() => { if (!cancelled) setResult((prev) => ({ ...prev, items: [] })); }), filters.query ? 250 : 0);
    return () => { cancelled = true; window.clearTimeout(timer); };
  }, [filters.query, filters.model, filters.node, filters.exception, filters.repair, filters.sort, workspace, result.page, result.size]);
  const rows = Array.isArray(result.items) ? result.items : [];
  ...
  return (
    <>
      ...
      <SimpleTabs items={FLOW_TABS} active={workspace} onChange={(value) => {
        setWorkspace(value);
        setFilters({ query: '', model: '', node: '', exception: '', repair: '', sort: 'desc' });
          ...
          {rows.map((device) => {
            ...
            return (
              <tr key={device.id} className="hover:bg-[#fafafa]">
                ...
                <td className="px-3 py-2 text-gray-700">{modelName(device, types)}</td>
