...
import ErpReferencePicker from '../components/ErpReferencePicker';
...
import { nowText, todayText } from '../data/dateTime';
import {
...
function DeviceResultForm({ batch, relations, devices, locations, onClose, onSave }) {
  ...
  const [form, setForm] = useState({
    ...
    actualDate: single?.actualDate || todayText(),
  ...
  return <div className="space-y-4">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
      ...
      <div><label className="block text-xs text-gray-600 mb-1">实际交付点位</label><Select className="w-full" value={String(form.actualLocationId ?? '')} onChange={(event) => setForm((prev) => ({ ...prev, actualLocationId: event.target.value }))}><option value="">暂不填写</option>{locations.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</Select></div>
    </div>
...
export default function DeliveryBatchDetail() {
  ...
  const revisionRows = relations.flatMap((relation) => {
    const histories = relation.resultHistory || [];
    return histories.map((history, index) => ({
      relation,
      history,
      index,
      previousResult: history.previousResult || history.originalResult || history.result,
      revisedResult: history.revisedResult || history.newResult || history.currentResult
        || histories[index + 1]?.result || relation.result,
    }));
  });
  const users = state.users.filter((item) => item.status !== '停用');
  const erpReferences = batch.erpReferences || [];
  const feishuLinks = batch.feishuLinks || [];
  const siteRecords = batch.siteRecords || [];
  const editingLink = modal?.type === 'feishu-edit' ? feishuLinks.find((item) => item.id === modal.id) : null;
  ...
  const updateBatch = (nextBatch, action, notes) => {
    const time = nowText();
    const request = dispatch({ type: 'UPDATE_DELIVERY_BATCH', payload: {
      planId: plan.id,
      batchId: batch.id,
      payload: { ...nextBatch, updatedAt: time, updatedBy: state.currentUser },
    } });
    Promise.resolve(request).then((saved) => {
      if (saved) dispatch({ type: 'ADD_OPERATION_LOG', payload: { id: createClientId('LOG'), deliveryPlanId: plan.id, batchId: batch.id, projectId: plan.projectId, actionType: action, module: '项目中心', notes: `${batchDisplayName(batch)}：${notes}` } });
    });
    setModal(null);
  };
  ...
  const saveLink = (form) => {
    const item = { id: editingLink?.id || createClientId('BFS'), name: form.name.trim(), url: form.url.trim(), note: form.note.trim() };
    const links = editingLink ? feishuLinks.map((link) => link.id === editingLink.id ? item : link) : [...feishuLinks, item];
    updateBatch({ ...batch, feishuLinks: links }, editingLink ? '编辑批次飞书链接' : '添加批次飞书链接', item.name);
  };
  const deleteLink = (link) => {
    if (!window.confirm(`确认删除“${link.name}”吗？`)) return;
    updateBatch({ ...batch, feishuLinks: feishuLinks.filter((item) => item.id !== link.id) }, '删除批次飞书链接', link.name);
  };
  const unlinkErp = (reference) => {
    if (!window.confirm(`确认解除 ${reference.type} ${reference.no} 的平台侧关联吗？`)) return;
    updateBatch({ ...batch, erpReferences: erpReferences.filter((item) => !(item.type === reference.type && item.no === reference.no)) }, '解除 ERP 来源关联', `${reference.type} ${reference.no}`);
  };
  ...
  const saveResults = (form) => {
    ...
    const nextRelations = relations.map((relation) => {
      ...
      const history = hadResult ? [...(relation.resultHistory || []), {
        previousResult: relation.result,
        revisedResult: form.result,
  ...
  return <Page>
    ...
    <Section title="关联记录">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div>
          ...
          {feishuLinks.length ? <div className="divide-y divide-gray-100">{feishuLinks.map((item) => <div key={item.id} className="flex items-start justify-between gap-3 py-2"><div><span className="text-[13px] text-gray-700">{item.name}</span>{item.note && <p className="text-xs text-gray-400 mt-0.5">{item.note}</p>}</div><div className="flex gap-3"><a className="ui-link text-[13px]" href={item.url} target="_blank" rel="noreferrer">打开表格</a><button className="ui-link text-[13px]" onClick={() => setModal({ type: 'feishu-edit', id: item.id })}>编辑</button><button className="ui-link text-[13px]" onClick={() => deleteLink(item)}>删除</button></div></div>)}</div> : <p className="text-xs text-gray-400 py-2">暂无当前批次补充链接。</p>}
        </div>
      </div>
    </Section>
    ...
    <Section id="batch-devices" title="本批次设备" right={<Btn size="sm" variant="primary" disabled={!selected.length} onClick={() => setModal({ type: 'result-batch' })}>批量录入结果（{selected.length}）</Btn>} bodyClassName="p-0">
      <Table head={['选择', '设备 SN', '机器人编号', '型号', '生产进度', 'ERP 产品入库', 'ERP 交付来源', '目标点位', '实际交付点位', '当前交付结果', '历史异常', '最近结果时间', '操作']} empty="本批次暂无设备">
        {relations.map((relation) => {
          ...
          return <tr key={relation.id} className="hover:bg-[#fafafa]">
            ...
            <td className="px-3 py-2">{device?.erpInboundNo ? '已关联' : '未关联'}</td><td className="px-3 py-2 text-xs">{erpReference ? <Link className="ui-link" to={erpReferenceUrl(erpReference, currentBatchUrl)}>{erpReference.no}</Link> : '—'}</td><td className="px-3 py-2">{targetLocation?.name || '—'}</td>
    ...
    {revisionRows.length > 0 && <Section title="设备结果修订记录" bodyClassName="p-0">
      <Table head={['设备 SN', '原结果', '当前结果', '原结果说明', '修改原因', '原记录人 / 时间', '修订时间']} empty="暂无结果修订">
        {revisionRows.map(({ relation, history, index, previousResult, revisedResult }) => {
          const device = state.devices.find((item) => item.id === relation.deviceId);
          return <tr key={`${relation.id}-${index}`} className="hover:bg-[#fafafa]"><td className="px-3 py-2 font-mono text-xs">{device?.sn || '—'}</td><td className="px-3 py-2"><StatusBadge status={previousResult} /></td><td className="px-3 py-2"><StatusBadge status={revisedResult} /></td><td className="px-3 py-2 text-xs text-gray-600">{history.resultSummary || history.exceptionDescription || '—'}</td><td className="px-3 py-2 text-xs text-gray-600">{history.modificationReason || '—'}</td><td className="px-3 py-2 text-xs text-gray-500">{history.recorder || '—'} / {history.recordTime || '—'}</td><td className="px-3 py-2 text-xs text-gray-500">{history.revisedAt || '—'}</td></tr>;
        })}
      </Table>
    </Section>}
    ...
    <Section title="操作日志" bodyClassName="p-0">
      <Table head={['操作时间', '操作人', '操作动作', '操作摘要']} empty="暂无批次操作日志">
        {state.operationLogs.filter((item) => item.batchId === batch.id).sort((a, b) => (b.timestamp || '').localeCompare(a.timestamp || '')).map((item) => <tr key={item.id} className="hover:bg-[#fafafa]"><td className="px-3 py-2 text-xs text-gray-500">{item.timestamp}</td><td className="px-3 py-2">{item.operator}</td><td className="px-3 py-2">{item.actionType}</td><td className="px-3 py-2 text-xs text-gray-500">{item.notes || '—'}</td></tr>)}
      </Table>
    </Section>
