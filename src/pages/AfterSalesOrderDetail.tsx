...
export default function AfterSalesOrderDetail() {
  ...
  return <Page>
    ...
    <Section title="现场处理" subtitle="现场工程师填写的实际处理事实，不覆盖来源问题快照。" right={!readOnly && order.status === '现场处理中' ? <Btn size="sm" onClick={() => setModal('material')}>补充资料</Btn> : undefined}>
      <DescList cols={2} items={[["实际处理方案", dash(order.actualSolution)], ...(order.finalResult ? [['历史最终处理结果', order.finalResult] as [ReactNode, ReactNode]] : []), ['是否涉及换件', order.involvesReplacement ? '是' : '否'], ...replacementItems]} />
    ...
    <Section title="关单信息"><DescList cols={3} items={[["关单人", dash(order.closedBy)], ['关单时间', dash(order.closedAt)], ...(order.closeNote ? [['历史关单说明', order.closeNote] as [ReactNode, ReactNode]] : []), ['取消人', dash(order.cancelledBy)], ['取消时间', dash(order.cancelledAt)], ['取消原因', dash(order.cancelReason)]]} /></Section>
    ...
    <CloseModal isOpen={modal === 'close'} order={order} issue={issue} onClose={() => setModal(null)} onSave={(values) => {
      const { causeLevel1, causeLevel2, causeLevel3, materials, ...handling } = values;
      ...
      update({ ...handling, status: '已关单', closedBy: state.currentUser || '当前用户', closedAt: nowText(), materials }, '完成并关单', '工单状态：现场处理中 → 已关单；售后现场处理已完成。');
...
function CloseModal({ isOpen, onClose, onSave, order, issue }: any) {
  const [values, setValues] = useState({
    actualSolution: order?.actualSolution || '',
  ...
  const submit = () => {
    if (!values.causeLevel1.trim()) return setError('请选择最终故障分类。');
    if (!values.actualSolution.trim()) return setError('请填写实际处理方案。');
    ...
    const materials = material ? [...order.materials, material] : order.materials;
    const missing = afterSalesCloseReasons({ ...order, ...values, materials });
    if (missing.length) return setError(`尚未满足：${missing.join('、')}。`);
    onSave({ ...values, materials });
  };
  return <FormModal isOpen={isOpen} onClose={onClose} title="完成处理并关单"><p className="text-xs text-gray-500">关单只结束本次现场服务，不会修改来源问题的质量跟进字段。</p><Field label="实际处理方案" required><Textarea value={values.actualSolution} onChange={(event) => set('actualSolution', event.target.value)} /></Field><div className="space-y-3 rounded-md border border-gray-200 p-3"><p className="text-xs font-medium text-gray-700">最终故障分类 <b className="text-red-500">*</b></p><div className="grid gap-3 sm:grid-cols-3"><CategorySelect label="一级故障原因分类" value={values.causeLevel1} options={ISSUE_CAUSE_LEVEL_1} onChange={(value: string) => set('causeLevel1', value)} /><CategorySelect label="二级故障原因分类" value={values.causeLevel2} options={ISSUE_CAUSE_LEVEL_2} onChange={(value: string) => set('causeLevel2', value)} /><CategorySelect label="三级故障分类" value={values.causeLevel3} options={ISSUE_CAUSE_LEVEL_3} onChange={(value: string) => set('causeLevel3', value)} /></div></div><label className="flex items-center gap-2 text-sm text-gray-700"><input type="checkbox" checked={values.involvesReplacement} onChange={(event) => setReplacement(event.target.checked)} />涉及换件</label>{values.involvesReplacement && <div className="space-y-3 rounded-md border border-gray-200 p-3"><Field label="物料名称" required><Input value={values.replacementMaterialName} onChange={(event) => set('replacementMaterialName', event.target.value)} /></Field><Field label="旧物料 SN" required><Input value={values.replacementOldSn} onChange={(event) => set('replacementOldSn', event.target.value)} /></Field><Field label="新物料 SN" required><Input value={values.replacementNewSn} onChange={(event) => set('replacementNewSn', event.target.value)} /></Field><Field label="换件说明"><Textarea value={values.replacementNote} onChange={(event) => set('replacementNote', event.target.value)} /></Field></div>}<AttachmentUpload value={proof} onChange={setProof} label="正常工作视频或处理结果证明资料" required />{error && <p className="rounded-md bg-amber-50 px-3 py-2 text-xs text-amber-800">{error}</p>}<div className="flex justify-end gap-2"><Btn onClick={onClose}>取消</Btn><Btn variant="primary" onClick={submit}>确认完成并关单</Btn></div></FormModal>;
}
