...
function NewBatchForm({ plan, project, state, onClose, onSave }) {
  ...
  const submit = () => {
    ...
    if (!form.owner.trim()) next.owner = '请输入批次负责人。';
    ...
    if (Object.keys(next).length) return setErrors(next);
    onSave({ ...form, owner: form.owner.trim(), sequence, baseName, displayName });
  };
  return (
    <div className="space-y-5">
      <div className="rounded-md border border-gray-200 p-4 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div><label className="block text-xs text-gray-600 mb-1">批次负责人 <span className="text-red-500">*</span></label><Input className="w-full" placeholder="请输入批次负责人姓名" value={form.owner} onChange={(event) => update('owner', event.target.value)} />{errors.owner && <p className="text-xs text-red-600 mt-1">{errors.owner}</p>}</div>
        </div>
      </div>
...
export default function DeliveryPlanDetail() {
  ...
  const persistedPlan = state.deliveryPlans.find((item) => item.id === id);
  const plan = persistedPlan || { id, projectId: '', plannedCount: 0, batches: [] };
  ...
  const updatePlan = (payload, action, notes) => {
    const time = nowText();
    const request = dispatch({ type: 'UPDATE_DELIVERY_PLAN', payload: { id: plan.id, ...payload, updatedAt: time, updatedBy: state.currentUser } });
    Promise.resolve(request).then((saved) => {
      if (saved) dispatch({ type: 'ADD_OPERATION_LOG', payload: { id: createClientId('LOG'), deliveryPlanId: plan.id, projectId: payload.projectId || plan.projectId, actionType: action, module: '项目中心', notes } });
    });
    setModal(null);
  };
  ...
  const saveBatch = (form) => {
    const time = nowText();
    const batchId = createClientId('BAT', plan.id, String(form.sequence).padStart(2, '0'));
    const batch = {
      id: batchId,
      ...
      erpReferences: form.erpReferences,
      feishuLinks: form.feishuName ? [{ id: createClientId('BFS'), name: form.feishuName.trim(), url: form.feishuUrl.trim(), note: '' }] : [],
      siteRecords: [],
  ...
  const logs = state.operationLogs.filter((item) => item.deliveryPlanId === plan.id)
    .sort((a, b) => (b.timestamp || '').localeCompare(a.timestamp || ''));
  if (!persistedPlan) return <Page><PageHeader title={state.projectCenterLoading ? '交付执行加载中' : '交付执行不存在'} actions={<Btn as="link" to="/projects?tab=delivery">返回列表</Btn>} /></Page>;
    ...
    <Page>
        ...
        <Table head={['设备 SN', '机器人编号', '型号', '所属批次', '目标点位', '实际点位', 'ERP 来源', '当前交付结果', '异常记录', '操作']} empty="暂无关联设备" footer={<Pagination {...devicePaged} onChange={devicePaged.setPage} onPageSizeChange={devicePaged.setPageSize} />}>
          {devicePaged.pageItems.map((relation) => {
            ...
            return <tr key={relation.id} className="hover:bg-[#fafafa]">
              ...
              <td className="px-3 py-2 text-xs text-gray-500">{erpReference ? <Link className="ui-link" to={erpReferenceUrl(erpReference, planDeviceReturn)}>{erpReference.no}</Link> : '—'}</td>
      ...
      {active === 'logs' && <Section title="操作日志" bodyClassName="p-0">
        <Table head={['操作时间', '操作人', '操作动作', '操作摘要']} empty="暂无操作日志">
          {logs.map((item) => <tr key={item.id} className="hover:bg-[#fafafa]"><td className="px-3 py-2 text-xs text-gray-500">{item.timestamp || '—'}</td><td className="px-3 py-2 text-gray-600">{item.operator || '—'}</td><td className="px-3 py-2 text-gray-700">{item.actionType}</td><td className="px-3 py-2 text-xs text-gray-500">{item.notes || '—'}</td></tr>)}
        </Table>
      </Section>}
