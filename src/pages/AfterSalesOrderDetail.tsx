...
export default function AfterSalesOrderDetail() {
  ...
  return <Page>
    <PageHeader title={order.orderNo} description="查看售后现场处理进展和来源问题。" breadcrumb={<div className="mb-1 text-xs text-gray-400"><Link className="ui-link" to="/after-sales?tab=orders">售后工单</Link> / {order.orderNo}</div>} actions={<Btn as="link" to="/after-sales?tab=orders">返回售后工单</Btn>} />
    ...
    <HandleModal isOpen={modal === 'handle'} order={order} onClose={() => setModal(null)} onSave={(values) => {
      ...
      update(handling, classificationChanged ? '售后现场处理更新故障分类' : handling.involvesReplacement ? '记录现场换件信息' : '记录现场处理', classificationChanged ? '已更新现场处理信息，并将最终故障分类同步至来源问题。' : handling.involvesReplacement ? `已更新现场处理信息；换前对象：${handling.replacementBefore}；换后对象：${handling.replacementAfter}${handling.replacementNote ? `；换件说明：${handling.replacementNote}` : ''}` : '已更新现场处理说明、实际处理方案和最终处理结果');
...
function HandleModal({ isOpen, onClose, onSave, order }: any) {
  const [values, setValues] = useState({
    ...
    involvesReplacement: Boolean(order?.involvesReplacement), replacementBefore: order?.replacementBefore || '', replacementAfter: order?.replacementAfter || '', replacementNote: order?.replacementNote || '',
