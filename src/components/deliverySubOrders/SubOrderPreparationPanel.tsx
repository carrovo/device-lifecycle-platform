    ...
    <Table head={['前置检查项', '要求', '状态', '完成时间', '说明', '资料', '操作']} empty="暂无前置检查项">
      {subOrder.checks.map((item) => {
        ...
        return <tr key={item.key} className={isCurrent ? 'bg-amber-50/50' : 'hover:bg-[#fafafa]'}>
          ...
          <td className="px-3 py-2"><StatusBadge status={item.status} /></td><td className="px-3 py-2 text-xs text-gray-500">{item.completedAt || '—'}</td><td className="px-3 py-2 text-xs text-gray-600">{item.note || '—'}</td><td className="min-w-44 px-3 py-2"><AttachmentList items={item.materials} empty="—" /></td>
